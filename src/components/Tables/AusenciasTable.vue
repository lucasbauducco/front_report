<template>
    <q-page padding>
      <div class="q-pa-md">
        <div class="report-page-title-container">
          <div class="report-page-title">Licencias</div>    
        </div>
              ...
              <template v-if="user_detail?.is_subadministrador || user_detail?.is_admin || user_detail?.is_staff" v-slot:body-cell-acciones="props">
                <q-td :props="props">
                  <q-btn
                    ...
                    icon="edit"
                    color="warning"
                    @click="editarAusencia(props.row)"
                  >
      ...
      <!-- Modal unificado de ausencia (crear/editar) -->
      <FormAusencia
        v-model="mostrarFormEditar"
        :ausencia="ausenciaEditando"
        @guardado="onAusenciaGuardada"
        @cancelado="onAusenciaCancelada"
      />
  
      <!-- Modal de creación de ausencia -->
      <FormAusencia
        v-model="mostrarFormCrear"
        @guardado="onAusenciaCreado"
        @cancelado="onAusenciaCreacionCancelado"
      />
    </q-page>
  </template>
  ...
  import FormAusencia from 'src/components/Forms/FormAusencia.vue'
  ...
  const ausenciaEditando = ref(null)
  ...
  const cargarAusencias = async () => {
    ...
    try {
      console.log('🔄 Cargando ausencias con filtros:', filtrosActuales.value)
      ...
      const response = await ausenciasService.getAusencias(params)
      
      console.log('✅ Respuesta recibida:', response)
      
      // Log del primer registro para debug
      if (response.results && response.results.length > 0) {
        console.log('📋 Primer ausencia (estructura):', JSON.stringify(response.results[0], null, 2))
      }
      
      ausencias.value = response.results
      totalRegistros.value = response.count
      pagination.value.rowsNumber = response.count
  
      $q.notify({
        type: 'positive',
        message: `Se cargaron ${response.results.length} ausencias`,
        position: 'top',
        timeout: 1500
      })
    } catch (error) {
      ...
      $q.notify({
        type: 'negative',
        message: error.response?.data?.detail || 'Error al cargar ausencias. Verifica que Django esté corriendo.',
        position: 'top',
        timeout: 3000
      })
    } finally {
  ...
  const verAusencia = (ausencia) => {
    const tarea = ausencia.tarea_nombre || 'Sin tarea';
    const observaciones = ausencia.observaciones || 'Sin observaciones';
...
const editarAusencia = async (ausencia) => {
    console.log('✏️ Editar ausencia:', ausencia)    
    ausenciaEditando.value = ausencia
    mostrarFormEditar.value = true
  }
  
  const onAusenciaGuardada = (ausenciaActualizada) => {
    console.log('✅ Ausencia guardada:', ausenciaActualizada)   
    // Recargar la lista de registros
    cargarAusencias()
  }
  
  const onAusenciaCancelada = () => {   
    ausenciaEditando.value = null
  } 
  
  const onAusenciaCreado = (resultado) => {
    console.log('✅ Ausencia(s) creada(s):', resultado)
    // Recargar la lista de ausencias
    cargarAusencias()
  }
  
  const onAusenciaCreacionCancelado = () => {
    // No hacer nada, solo cerrar el modal
  }
  
  const eliminarAusencia = async (ausencia) => {
    $q.dialog({
      title: 'Confirmar eliminación',
      message: `¿Estás seguro de que deseas eliminar la ausencia #${ausencia.id}?`,
      cancel: {
        label: 'Cancelar',
        color: 'grey',
        flat: true
      ...
      try {
        console.log('🗑️ Eliminando ausencia:', ausencia.id)
        
        await ausenciasService.deleteAusencia(ausencia.id)
        ...
        $q.notify({
          type: 'negative',
          message: error.response?.data?.detail || 'Error al eliminar ausencia',
          position: 'top',
          timeout: 3000
        })
      }
  ...
  const verificarArchivoExcel = async () => {
    try {
    const response = await ausenciasService.getExcelAusencias()
  ...
  const descargarExcelExistente = async () => {
    ...
    const response = await ausenciasService.downloadExcelAusencias()
 ...
 // Computed para filtrar ausencias según el estado de mostrarInactivos
    const ausenciasFiltradas = computed(() => {
    if (mostrarInactivos.value) {
      // Si está activado, mostrar todos (activos e inactivos)
      return ausencias.value
    } else {
      // Por defecto, solo mostrar activos
      return ausencias.value.filter(ausencia => ausencia.activo === true)
    }
  })
  
  // Cargar ausencias al montar el componente
  onMounted(() => {
    console.log('🚀 Componente AusenciasPage montado, cargando ausencias...')
    cargarAusencias()
    verificarArchivoExcel()
  })
  </script>
